...
export default function GuestList() {
  ...
  if (isLoading) {
    return <div className="flex justify-center my-8 text-gray-900">Loading guests...</div>;
  }
  
  if (error) {
    return (
      <div className="my-8 p-4 bg-red-100 text-gray-900 rounded-md">
  ...
  if (guests.length === 0) {
    return <div className="text-center my-8 text-gray-900 font-medium">No guests have registered yet.</div>;
  }
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <PrimaryButton onClick={fetchGuests} size="sm">
          Refresh
        </PrimaryButton>
      </div>
        ...
        <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="py-3 px-4 text-left text-gray-900">Purpose</th>
              <th className="py-3 px-4 text-left text-gray-900">Check In</th>
              <th className="py-3 px-4 text-left text-gray-900">Check Out</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {guests.map((guest) => (
              <tr key={guest.id} className="hover:bg-gray-50">
                ...
                <td className="py-3 px-4 text-gray-900">{guest.email}</td>
                <td className="py-3 px-4 text-gray-900">{guest.purpose}</td>
                <td className="py-3 px-4 text-gray-900">{formatDate(guest.check_in)}</td>
